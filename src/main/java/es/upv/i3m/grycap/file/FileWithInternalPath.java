...
public final class FileWithInternalPath implements File {
  ...
  // find a pattern like 'file(path_fo_file)' and divide it in three groups
  ...
  /**
   * Read the file specified and Check for strings of type 'file(' inside the
   * file passed and puts the content of the referenced file in the string
   * returned.
   *
  ...
  public String read() throws FileException {
    return lookForFilePaths(this.file.read());
  }
   ...
   * in the path.
   */
  private static String lookForFilePaths(final String fileContent)
      throws FileException {
    String result = fileContent;
    Pattern ptrn = Pattern.compile(FILE_PATH_PATTERN);
    Matcher matcher = ptrn.matcher(fileContent);

    while (matcher.find()) {
      result = replacePatternByFileContent(result, matcher);
    }
    return result;
  }
  ...
  private static String replacePatternByFileContent(final String fileContent,
    ...
    return fileContent.replace(matcher.group(), nestedFileContent);
  }

  /**
   * Return the content of the file path passed with the newlines escaped twice
   * (Design decision related to the IM REST API).
   */
