...
public class InfrastructureManagerApiTest extends GenericTestWatcher {
  ...
  public static void setRestClient() {
    try {
      imApiClient = new InfrastructureManagerApi(IM_DUMMY_PROVIDER_URL,
          AUTH_FILE_PATH);
  ...
  public void testReconfigureSomeVmsRadl()
    ...
    response = getImApiClient().reconfigure(getInfrastructureId(),
        new Utf8File(RADL_ALTER_VM_FILE_PATH).read(),
        BodyContentType.RADL, Arrays.asList(0, 1));
    checkServiceResponse(response);
  }
  ...
  public void testRestApiBodyContentType() throws ImClientException {
    if (!BodyContentType.TOSCA.equals(BodyContentType
        .getEnumFromValue(BodyContentType.TOSCA.toString()))) {
      Assert.fail();
    }
  ...
  public void testAuthorizationFileWithInternalPaths()
      throws ImClientException {
    InfrastructureManagerApi imApiClient =
        new InfrastructureManagerApi(IM_DUMMY_PROVIDER_URL,
